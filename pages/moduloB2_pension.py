...
# MoDULO B — PROYECCIoN DE RETIRO O PENSIoN MENSUAL

st.title("🧮 Modulo B — Proyeccion de retiro o pension mensual")

# 1️⃣ Entrada del saldo final acumulado (desde Modulo A o manual)
...
st.info("Este monto proviene del modulo A (o puedes ingresarlo manualmente).")

# 2️⃣ Eleccion del tipo de jubilacion
st.subheader("2️⃣ Tipo de jubilacion")
opcion = st.radio(
    "Selecciona una opcion:",
    ("Retiro total (un solo pago)", "Pension mensual (renta periodica)")
)

# 3️⃣ Parámetros de cálculo
st.subheader("3️⃣ Parámetros financieros")
...
años_retiro = st.number_input(
    "Años de duracion estimada de la jubilacion:",
    min_value=1,
    value=20,
    step=1
)
...
# 4️⃣ Cálculo principal
st.subheader("4️⃣ Resultados")

if opcion == "Retiro total (un solo pago)":
    ganancia = saldo_final * tasa_retorno
    impuesto = ganancia * tasa_impuesto
    monto_final = saldo_final + ganancia - impuesto

    st.success(f"💵 Monto total disponible: **${monto_final:,.2f} USD**")
    st.caption(f"(Incluye ganancia de ${ganancia:,.2f} y se aplico un impuesto de ${impuesto:,.2f})")

else:
    # Pension mensual calculada con formula de renta financiera
    ...
    st.success(f"💰 Pension mensual estimada: **${pension_mensual:,.2f} USD**")
    st.write(f"Total estimado recibido en {años_retiro} años: **${total_neto:,.2f} USD netos**")
    st.caption(f"(Impuesto aplicado: ${impuesto:,.2f})")

# 5️⃣ Comparador de escenarios
st.subheader("5️⃣ Comparar escenarios")
...
if st.button("🔍 Comparar escenarios"):
    # Suponiendo que capital crece adicionalmente con los años de diferencia
